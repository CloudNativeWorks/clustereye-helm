...
#################################################################################
# ClusterEye Stack Installation Script
#
# Description: Automated installation script for ClusterEye database monitoring platform
#              on Ubuntu 24.04 with kind (Kubernetes in Docker)
#
# Usage: ./install.sh
# The script will interactively prompt for configuration:
#   - Frontend domain (e.g., ui.example.com)
...
readonly CLUSTER_NAME="clustereye-cluster"
readonly CLUSTER_NAMESPACE="clustereye"
readonly HELM_REPO_NAME="clustereye"
readonly HELM_REPO_URL="https://charts.clustereye.com/"
readonly HELM_CHART_NAME="clustereye-stack"
...
║                          ╚══════╝   ╚═╝   ╚══════╝                  ║
║                                                                     ║
║               ClusterEye Stack Installation Script v1.0.1           ║
║               Database Monitoring & Management Platform             ║
╚═════════════════════════════════════════════════════════════════════╝
EOF
    echo -e "${COLOR_RESET}\n"
}
...
check_prerequisites() {
    ...
    if [ "$available_space_gb" -lt "$required_space_gb" ]; then
        ...
        log_info "  • ClusterEye stack containers: ~3-4GB"
...
prompt_configuration() {
    ...
    echo -e "${COLOR_CYAN}Please provide the following configuration details:${COLOR_RESET}"
    echo ""

    # Frontend Domain
    echo -e "${COLOR_YELLOW}1. Frontend Domain${COLOR_RESET}"
    echo "   The domain where ClusterEye UI will be accessible"
    echo "   Example: ui.example.com, clustereye.mycompany.com"
    echo ""
    while true; do
        read -p "   Enter frontend domain: " FRONTEND_DOMAIN
    ...
    # API Domain
    echo -e "${COLOR_YELLOW}2. API Domain${COLOR_RESET}"
    echo "   The domain where ClusterEye API will be accessible"
    echo "   Example: api.example.com, api.clustereye.mycompany.com"
    echo ""
    while true; do
    ...
    # SSL Configuration
    echo -e "${COLOR_YELLOW}3. SSL/HTTPS Configuration${COLOR_RESET}"
    echo "   Enable HTTPS for secure connections?"
    echo "   Note: You'll need to configure certificates separately if enabled"
    echo ""
    while true; do
        read -p "   Enable SSL? (y/N): " SSL_RESPONSE
        SSL_RESPONSE="${SSL_RESPONSE:-n}"
        case "$SSL_RESPONSE" in
            [Yy]* ) SSL_ENABLED="true"; break;;
            [Nn]* ) SSL_ENABLED="false"; break;;
    ...
    echo ""
    echo -e "  Frontend URL:     ${COLOR_GREEN}${protocol}://${FRONTEND_DOMAIN}${COLOR_RESET}"
    echo -e "  API URL:          ${COLOR_GREEN}${protocol}://${API_DOMAIN}${COLOR_RESET}"
    echo ""
...
EOF
    ...
    log_info "  gRPC port: 50051 (for agents)"
    echo
...
setup_helm_repo() {
    ...
    log_info "Adding ClusterEye Helm repository..."
    helm repo add "$HELM_REPO_NAME" "$HELM_REPO_URL" 2>/dev/null || \
        helm repo add "$HELM_REPO_NAME" "$HELM_REPO_URL" --force-update

    # Add InfluxDB Helm repository for dependencies
    log_info "Adding InfluxDB Helm repository..."
    helm repo add influxdata https://helm.influxdata.com/ 2>/dev/null || \
    ...
    log_success "Helm repositories configured successfully"

    # List available charts
    log_info "Available ClusterEye charts:"
...
install_clustereye_stack() {
    log_step "Installing ClusterEye Stack"
...
EOF
    ...
    log_warning "Please save these credentials and delete the file!"
...
print_summary() {
    ...
    echo ""
    echo -e "${COLOR_CYAN}ClusterEye Configuration:${COLOR_RESET}"
    echo "  Frontend URL:       ${protocol}://${FRONTEND_DOMAIN}"
    echo "  API URL:            ${protocol}://${API_DOMAIN}"
    echo "  SSL Enabled:        ${SSL_ENABLED}"
    echo "  gRPC Port:          50051 (for agents)"
    echo ""
    echo -e "${COLOR_CYAN}Default Login:${COLOR_RESET}"
    echo "  Username:           admin"
    ...
    echo ""
    echo "  # View API logs"
    echo "  kubectl logs -n ${CLUSTER_NAMESPACE} -l app.kubernetes.io/name=clustereye-api --tail=100 -f"
    echo ""
    echo "  # List Helm releases"
    echo "  helm list -n ${CLUSTER_NAMESPACE}"
    echo ""
    echo "  # Upgrade ClusterEye"
    echo "  helm upgrade ${HELM_CHART_NAME} ${HELM_REPO_NAME}/${HELM_CHART_NAME} -n ${CLUSTER_NAMESPACE}"
    echo ""
    echo "  # Delete cluster"
    echo "  kind delete cluster --name ${CLUSTER_NAME}"
    echo ""
    echo -e "${COLOR_CYAN}Agent Installation:${COLOR_RESET}"
    echo "  To monitor databases, install ClusterEye agent on your database servers."
    echo "  Visit: ${protocol}://${FRONTEND_DOMAIN}/agents for agent installation instructions"
    echo ""
    echo -e "${COLOR_CYAN}Next Steps:${COLOR_RESET}"
    echo -e "  1. Access ClusterEye UI at: ${COLOR_GREEN}${protocol}://${FRONTEND_DOMAIN}${COLOR_RESET}"
    echo "  2. Login with default credentials (admin/admin)"
    echo "  3. Change your password immediately"
    echo "  4. Add your database servers via agents"
    echo "  5. Configure alerts and monitoring thresholds"
    echo ""
    ...
    echo ""
    echo -e "${COLOR_YELLOW}Credentials:${COLOR_RESET} Installation credentials saved to /tmp/clustereye-credentials.txt"
    echo "      Please save them securely and delete the file!"
    echo ""
    echo "For more information, visit: https://docs.clustereye.io"
    echo ""
    echo -e "${COLOR_GREEN}Happy monitoring!${COLOR_RESET}"
}
...
main() {
    ...
    log_success "All done! ClusterEye stack is ready to use."
}
